...
class StartScreen(QWidget):
    scan_requested = pyqtSignal(str, list, bool, bool, bool)  # host, cves, port, nmap, subfinder
    ...
    def _build(self):
        ...
        opts = QHBoxLayout()
        opts.setSpacing(24)
        self.chk_subfinder = self._chk("Subfinder (서브도메인)")
        self.chk_port      = self._chk("Naabu (포트 스캔)")
        self.chk_nmap      = self._chk("Nmap 서비스 탐지")
        self.chk_subfinder.setChecked(False)
        self.chk_port.setChecked(True)
        self.chk_nmap.setChecked(True)
        opts.addWidget(self.chk_subfinder)
    ...
    def _on_start(self):
        ...
        self.scan_requested.emit(
            host, selected,
            self.chk_port.isChecked(),
            self.chk_nmap.isChecked(),
            self.chk_subfinder.isChecked(),
        )
